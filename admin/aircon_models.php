...
try {
    ...
    if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['add_aircon_model'])) {
        ...
        if ($brand && $model_name && is_numeric($hp) && is_numeric($price)) {
            $stmt = $pdo->prepare("INSERT INTO aircon_models (brand, model_name, hp, price) VALUES (?, ?, ?, ?)");
            $stmt->execute([$brand, $model_name, $hp, $price]);
    ...
    if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['edit_aircon_model'])) {
        ...
        if ($brand && $model_name && is_numeric($hp) && is_numeric($price)) {
            $stmt = $pdo->prepare("UPDATE aircon_models SET brand = ?, model_name = ?, hp = ?, price = ? WHERE id = ?");
            $stmt->execute([$brand, $model_name, $hp, $price, $model_id]);
    ...
    <div class="card mb-4">
        <div class="card-body">
            ...
            <form method="post" class="row g-3">
                ...
                <div class="col-md-2">
                    <input type="number" name="hp" class="form-control" placeholder="HP" step="0.1" min="0" required>
                </div>
                <div class="col-md-3">
                        ...
                        <?php if (count($airconModels)): ?>
                            <?php foreach ($airconModels as $model): ?>
                                <tr>
                                    ...
                                    <td><?= htmlspecialchars($model['hp']) ?>HP</td>
                            ...
                            <?php endforeach; ?>
                        <?php else: ?>
                            <tr><td colspan="6" class="text-center">No aircon models found.</td></tr>
                        <?php endif; ?>
                    </tbody>
                    </table>
                </div>
                ...
                <form method="post" class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="number" name="hp" class="form-control" value="<?= htmlspecialchars($model['hp']) ?>" step="0.1" min="0" required>
                        </div>
                        <div class="mb-3">
